...
- [ ] **Integrate snapshot-average play.** Retain the profiles actually used during collection, including uniform bootstrap, with correct own-reach weighting and one snapshot per hand. Keep current regret-matched collection separate from average-policy evaluation. Check complete historical actions, variable seats and export provenance before relying on averaged play.
...
- **Next task:** integrate snapshot-average play for the no-limit learner. Retain the profiles actually used for collection, including the initial uniform policy; verify own-reach-weighted queries and fixed-per-hand snapshot sampling against controlled histories. Keep average-policy inference distinct from the current profile used by the collector.
- **Following tasks:** checkpoint the complete no-limit training and average state, then verify exact resume. Before another learning campaign, integrate snapshot-aware provenance/recovery and declare fresh readiness checks. Substantial training requires a readiness pass and the corrected no-limit pipeline. Profile before a budgeted architecture pilot. Prior-hand opponent adaptation remains milestone 7; the current-hand encoder preserves those records in its source observation.
