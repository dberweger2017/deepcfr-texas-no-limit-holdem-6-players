...
- **Next task:** diagnose the remaining strategy-fitting error from the final snapshots from twelve exploration seeds. Compare neural predictions with replay-weighted targets by information set, including absent and rarely sampled decisions, and use the full weighted objective to distinguish optimization error from representation/coverage limits. Choose the next bounded fitting change from that evidence, then freeze its recipe before fresh confirmation. Keep the original tolerances and all failed campaigns. Do not begin the no-limit learning rewrite until milestone 3 passes.
