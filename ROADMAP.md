...
- **Next task:** write a bounded decision record for production strategy fitting and its fresh-confirmation criteria, using the [completed fitting study](docs/reports/strategy-fitting.md). Decayed minibatches pass every absolute-limit check but fail the additional paired-regression guardrail on seed 239, replicate 0. Compare the cost of integrating this schedule with an alternative such as Single Deep CFR; resolve that protocol decision before more training. Any proposed criterion change must be prospective and explicit, while this screen remains failed. Do not launch another width sweep, use reserved confirmation seeds, or begin the no-limit learning rewrite until the milestone-3 decision and gate are resolved.
