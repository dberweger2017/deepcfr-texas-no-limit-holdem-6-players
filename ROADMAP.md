...
- [ ] **Meet the neural-versus-tabular convergence gate.** [PR #48](https://github.com/dberweger2017/deepcfr-texas-no-limit-holdem-6-players/pull/48) delivers complete recovery and the first multi-seed comparison. Kuhn passes; Leduc fails. [PR #50](https://github.com/dberweger2017/deepcfr-texas-no-limit-holdem-6-players/pull/50) adds independent strategy capacity and a longer twelve-seed study. Wider fits improve all final results but still fail on seed 211; confirmation remains unused. Diagnose strategy-fitting errors by information set using the retained replay, then freeze a revised recipe before fresh confirmation with the original limits. Consider Single Deep CFR as a separately benchmarked alternative after this baseline works.
...
- **Next task:** implement and validate the [frozen strategy-fitting experiment](docs/strategy-fitting-study.md), then execute it within its $2 CPU ceiling. Compare minibatch and exact-objective fitting, each with fixed and decayed learning rates, on all twelve saved final replays and three fitting replicates. Preserve the original weighted objective and input hashes; inspect errors by decision situation. Exact-objective fits are diagnostic controls, and fresh confirmation seeds remain unused until a production recipe is frozen. Keep the original convergence limits and all failed campaigns. Do not begin the no-limit learning rewrite until milestone 3 passes.
