...
- [ ] **Meet the neural-versus-tabular convergence gate.** [PR #48](https://github.com/dberweger2017/deepcfr-texas-no-limit-holdem-6-players/pull/48) adds complete replay/RNG recovery and the first predeclared multi-seed comparison. Kuhn passes; Leduc misses the required all-seed tolerance. Improve strategy fitting and investigate coverage using the retained diagnostics, then declare a new confirmation campaign. Consider Single Deep CFR as a separately benchmarked alternative after this baseline works.
...
- **Next task:** address the measured Leduc strategy-fitting and replay-coverage limitations, then run a newly declared multi-seed convergence campaign. Keep the failed campaign as the baseline and require the original small-game tolerances before the no-limit learning rewrite or substantial training. Evaluate Single Deep CFR separately if the evidence warrants it.
- **CPU pilot:** [Runpod protocol](docs/cpu-pilot.md), with an owner-approved $10 total budget. Compare one and four independent workers, then diagnose the three retained Leduc replays. Preserve failed results and keep milestone 3 open until a fresh convergence campaign passes.
