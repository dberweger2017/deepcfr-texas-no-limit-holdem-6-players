...
- **Benchmark infrastructure delivered:** varied legal styles, separate named training/evaluation pools, four-/five-/six-player stack-depth and session plans, and frozen standard checkpoint adapters with verified bytes and public-only decisions. See the [benchmark contract](docs/benchmarks.md) and [declared validation/results](docs/reports/benchmark-validation.md). Historical training seeds are unknown; multi-seed learned-opponent campaigns remain open.
- **Next task:** implement the tabular two-player Kuhn and Leduc reference with exact best-response evaluation. Check game utilities, regret updates, strategy averaging, and external sampling against small exact calculations, and declare convergence thresholds before acceptance runs. This is the gate before rewriting neural CFR or doing substantial training.
