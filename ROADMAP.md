...
- **Next task:** give the strategy network independent capacity and test width 128 with 24,000 fitting steps while preserving advantage training. Declare a fresh multi-seed convergence campaign with the original tolerances. Keep the failed campaign as the baseline; do not begin the no-limit learning rewrite until milestone 3 passes. Coverage remains a diagnostic concern; evaluate Single Deep CFR separately if the evidence warrants it.
