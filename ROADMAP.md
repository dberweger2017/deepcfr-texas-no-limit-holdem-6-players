...
- [x] **Integrate snapshot-aware training and recovery.** Complete archive/replay/RNG checkpoints, fresh-process equivalence and exported-policy evaluation are implemented. The [fresh snapshot protocol](docs/snapshot-readiness.md) declares eight new seeds per game, unchanged absolute limits and a $1.50 / two-hour CPU ceiling. Inference exports cannot resume training. The campaign result is pending; this checkbox records implementation only.
...
**Engineering status:** the planned implementation tasks are delivered. The learning exit condition below remains open while milestone 3 readiness is failed; short implementation runs cannot close it.
...
- **Repository cleanup:** the old trainers, opponent model, desktop UI, notifier and obsolete package/release entrypoints are retired. [Repository layout](docs/repository-layout.md) lists current commands and the preserved historical adapter. Rules, observation/session tests and research artifacts remain. This maintenance change does not close a learning gate.
...
- **Remaining Hold'em gaps:** the legacy trainer still has limited history inputs, self-imitation sizing targets, deviations from validated CFR sampling/averaging, and incomplete training recovery and competitive benchmark coverage. The replacement decision encoder, legal bet/target interfaces and all-role collector are available. Role-specific fitting, averaged play and complete iteration-boundary recovery are implemented. The first short multi-seed arena report is available; small-game readiness, substantial Hold’em learning and competitive benchmark coverage remain open.
...
- **Next task:** execute the committed [snapshot readiness protocol](docs/snapshot-readiness.md), retaining every declared seed and failure. Archive-aware recovery and export evaluation are implemented; the fresh learning gate is pending. The previous failed confirmation stays failed. The separate cleanup retires obsolete pre-rewrite workflows; useful regressions and a narrow historical inference adapter remain.
- **Following tasks:** after readiness passes, produce a meaningful Hold’em learning baseline, then profile collection/fitting/archive costs before the milestone 5 architecture and hardware pilot. Substantial training and default-model promotion still require their respective gates. Prior-hand opponent adaptation remains milestone 7.
