...
**Bounded follow-up:** the one frozen confirmation campaign is complete and failed. Make a strategy-extraction architectural decision, including a Single Deep CFR comparison with explicit weighting, storage/inference costs and acceptance checks. All final exact played averages pass, but Leduc 433 has little margin and its replay average fails. Do not assume a better strategy fit alone solves readiness. No automatic extra seeds, steps or widths. Independent Hold'em engineering can proceed, while scaled training remains gated.
...
- **Next task:** make the bounded strategy-extraction decision, including a Single Deep CFR design/cost comparison and exact small-game acceptance checks. Reuse saved diagnostics; do not automatically run more training or reuse consumed seeds as fresh confirmation. The [earlier decision record](docs/decisions/strategy-fitting-next.md) remains the historical rationale for the now-completed prospective test. This roadmap itself starts no rental or unattended work.
- **Next Hold’em tasks:** begin full public-history/variable-seat encoding with independent correctness checks, then legal bet candidates with separate regret targets, consistent all-role self-play and complete recovery. Engineering may proceed while milestone 3 remains open; substantial training requires a readiness pass and the corrected no-limit pipeline. Profile before a budgeted architecture pilot.
