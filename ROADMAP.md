...
- [ ] **Integrate strategy learning-rate decay.** Add an explicit strategy-only cosine schedule with constant-mode equivalence, complete configuration/provenance, and deterministic recovery checks. Keep advantage fitting and collection unchanged. See the [decision record](docs/decisions/strategy-fitting-next.md).
- [ ] **Run one fresh end-to-end confirmation.** Freeze executable configurations, all eight reserved seeds, final-only checks, paired diagnostics, and a resource cap before running. Every seed must meet the original absolute limits: Kuhn exploitability/value error ≤0.03/0.03; Leduc ≤0.15/0.10. The prior exploration's 0.005 paired-regression veto is explicitly not carried into this prospective readiness test; regressions remain reported, and the old screen remains failed. Passing readiness does not establish uniform improvement or promote a Hold'em model.

**Bounded follow-up:** one frozen confirmation campaign follows the integration. Failure triggers an architectural decision, including whether Single Deep CFR is a better strategy-extraction approach; it does not automatically trigger more seeds, steps, or widths. Independent Hold'em engineering can proceed, while scaled training remains gated.
...
**Engineering can start while milestone 3 confirmation is pending.** Work on decision encoding, action generation, trainer interfaces, and recovery has independent acceptance checks. Substantial self-play and architecture campaigns require both the small-game readiness pass and a correct no-limit pipeline. Do not run a capacity sweep on the legacy trainer.
...
Vast.ai remains a candidate for substantial training. The owner authorized a **$10 total Runpod CPU budget** on September 15, 2026, including more cores and longer small-game runs, separate from future GPU funding. The [bounded CPU pilot](docs/cpu-pilot.md) used only a few cents. The capacity and fitting studies are complete with conservative total CPU usage of $1.43, leaving $8.57 of that authorization. All rentals are terminated; confirmation remains unused. New CPU runs still require a committed protocol and a cap within the remaining allowance. GPU funding is separate. Before a rental beyond that authorization, present a fresh cost/performance comparison and obtain the owner's campaign budget. The comparison must include:
...
- **Next task:** integrate explicit cosine strategy fitting and its recovery/equivalence checks, then commit the full fresh-confirmation protocol. The [decision record](docs/decisions/strategy-fitting-next.md) chooses this small change over another width sweep or immediate Single Deep CFR rewrite, and explicitly separates the old failed exploration screen from prospective readiness criteria. No new rental is started by this roadmap.
- **Next Hold’em tasks:** full public-history/variable-seat encoding, legal bet candidates with separate regret targets, consistent all-role self-play, and complete recovery. Independent engineering is allowed while small-game confirmation is pending; substantial training remains gated. After readiness, profile and run a budgeted architecture pilot on the corrected learner.
