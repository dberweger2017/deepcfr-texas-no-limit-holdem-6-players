...
- **Remaining Hold'em gaps:** the legacy trainer still has limited history inputs, self-imitation sizing targets, deviations from validated CFR sampling/averaging, and incomplete training recovery and competitive benchmark coverage. The replacement decision encoder is now available; useful bet candidates, all-role self-play and integration with validated learning/recovery remain ahead.
...
- **Next task:** generate legal street-appropriate bet candidates, including minimum raises, overbets and exact all-ins, deduplicate after applying integer bounds, and give each candidate a distinct regret/value output and target. Use the new decision encoding and exact observation bounds; do not revive the legacy self-imitation sizing head.
