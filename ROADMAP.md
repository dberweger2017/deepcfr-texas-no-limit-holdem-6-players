...
- **Remaining gaps:** limited neural history encoding, fixed-size seat inputs, self-imitation sizing targets, deviations from validated CFR sampling/averaging, and incomplete training/evaluation reproducibility.
- **Next task:** add reproducible evaluation schedules and reports: independent random streams, declared seat/deal rotations, disjoint training/validation/test schedules, hand-level outcomes, a reproducibility manifest, and uncertainty over independent deal or session blocks. Keep fixed-stack hand benchmarks distinct from bankroll sessions. No serious training until the arena and small-game solver checks are in place.
