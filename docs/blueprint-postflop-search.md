...
The search retains the blueprint's raise menu. It does not add arbitrary new raise sizes for the player. It **does** preserve an opponent's actual off-menu wager in the reconstructed game and posterior update, avoiding a fictitious nearby wager. Leaf profiles are sampled rather than optimized by CFR, and the private ranges use 96 candidates per seat rather than all 1,326 combinations. These approximations are explicit risks for both playing strength and latency.
