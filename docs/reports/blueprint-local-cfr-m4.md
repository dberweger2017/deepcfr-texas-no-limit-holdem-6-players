...
without an invalid action. Peak process RSS was **7.59 GiB** under the 10.5-GiB
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Scripted styles | 128 / 1,536 | −252.67 [−532.30, +26.96] | −294.24 [−574.54, −13.93] | +41.57 [−90.40, +173.54] | 78/78 | 3.85 / 4.39 s |
| Random | 64 / 768 | +513.67 [−239.15, +1,266.50] | +484.64 [−271.70, +1,240.97] | +29.04 [−19.68, +77.75] | 19/19 | 3.97 / 3.99 s |

All completed solves used the configured 128 cycles. The run recorded about
2.19 million sampled traversal nodes, 867,951 leaf choices, and 1,088 public
...
continuations: only 37,122 of 1,982,158 scripted continuation lookups used a
...
limit. The final run on the **same** frozen schedules completed 97/97; its
scripted paired effect changed from +42.81 [−91.20, +176.81] to +41.57
[−90.40, +173.54]. This repair happened after the first result was inspected.
Treat the final paired intervals as exploratory descriptions, not fresh
confirmatory evidence. Both earlier runs and their artifacts are retained on
the M4.

## Verification and artifacts

The final M4 run and a second run at source revision
`9df1d39a54d2cd2d8c78bfd06486c2ec3ea423c1` produced byte-identical
...
SHA-256 values are
`79a8d8d78b02b2b1fac34931c4df795d9db2c47aaf34f3c585684258a04d4a15`
and `bc48513e55871eb7552a1ee5067cf80a9e881703e692e0b571ae5c920fc96bf0`.
The [compact JSON result](blueprint-local-cfr-m4.json) preserves rates,
telemetry, source and checkpoint hashes, and artifact locations.

Raw hand rows, per-attempt solver records, manifests, and TensorBoard events
remain in `~/Local/blueprint-local-cfr/results/blueprint-local-cfr-m4-final-20260925/`
and its `-final-replay-20260925` companion on the M4. Hash-verified copies
...
The focused local solver/search suite passed 19 tests. The complete M4 suite
passed 781 tests at the initial code revision, and final PR CI passed after
the depth-limit fix. One unrelated local-Mac full-suite test hit its existing
