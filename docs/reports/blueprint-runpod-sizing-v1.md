...
The account's imported SSH key did not match any available private key on the local Mac or M4, so no commands or checkpoint transfer ran inside this pod. It was stopped with the console showing **$0.00/hour** continuing cost. The displayed balance moved from **$18.07 to $18.02** after delayed billing settled, an approximately **$0.05** charge. No scientific result or full topology measurement came from this first attempt. Adding this Mac's public key to the account is pending the required security-access confirmation; the pod will be started again only after access is resolved.
