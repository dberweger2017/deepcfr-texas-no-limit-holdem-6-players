...
class _TablePolicy:
    def __init__(self, trainer: BlueprintTrainer, seed: int, counts: Counter, *, uniform: bool):
    ...
    def choose_action(self, view):
        ...
        if not self.uniform:
            self.counts[(view.street.value, f"action_{selected.action.kind.value}")] += 1
        return selected.action


def evaluate(trainer: BlueprintTrainer, plan: Plan) -> dict:
    ...
    if plan.models or any(s.mode != "fixed" or len(s.stacks) != trainer.table.capacity for s in plan.scenarios):
        raise ValueError("Blueprint evaluation requires fixed matching tables and no artifacts")
    counts: Counter = Counter()
    rows = []

    def factory(name, seed):
        if name == "blueprint_live":
            return _TablePolicy(trainer, seed, counts, uniform=False)
    ...
    def emit(row, _timing):
        ...
        compact = {key: value for key, value in row.items() if key not in {"events", "reloads", "outcome_sha256"}}
