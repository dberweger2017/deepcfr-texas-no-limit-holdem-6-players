...
**Full-game learning remains unproven.** The [paired policy comparison](docs/reports/holdem-policy-comparison.md) found no consistent fitting improvement, and the [persistent critic](docs/reports/holdem-persistent-critic.md) failed its cost screen. The [additional branching study](docs/reports/holdem-collector-branching.md) gives a promising collector result: expanding a second own decision reduces variance × computation by roughly 93–95% on controlled frozen probes. It does not resolve sparse postflop coverage, which is also visible under the initial uniform policy. The [trainer integration and local pilot](docs/reports/holdem-branching-pilot.md) now pass recovery checks. **Next: compare fresh online learning with the original collector on Runpod, using a fixed multi-seed protocol and spending cap.** Held-out card generalization remains an open issue from the [river diagnostic](docs/reports/holdem-river-reference.md). Artifacts remain local; public hosting and v0.5 release packaging are ahead. No production model has been promoted, and the v1.0 professional standard remains unproven.
