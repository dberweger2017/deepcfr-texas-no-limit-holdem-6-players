...
**Next step:** reduce sampling variance and define the corresponding training loss before integrating a new collector. The [sampling comparison](docs/reports/holdem-sampling-comparison.md) finishes the difficult root cheaply with sampled paths, but reveals extreme late-decision update weights. Exact expectation checks pass; efficient, reliable neural learning is still unproven. Step 4's collection → fitting → averaged play → evaluation pipeline and recovery are implemented, while its meaningful multi-seed learning gate remains open. [Small-game snapshot readiness](docs/reports/snapshot-readiness.md) has passed. No Hold'em model has been promoted.
