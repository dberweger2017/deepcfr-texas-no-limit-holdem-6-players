...
**Full-game learning remains unproven.** The [paired policy comparison](docs/reports/holdem-policy-comparison.md) found no consistent fitting improvement. The subsequent [six-player river diagnostic](docs/reports/holdem-river-reference.md) shows that the current network can recover reliable training targets and an exact baseline substantially reduces sampling noise, while retaining costly held-out card errors. The [persistent critic comparison](docs/reports/holdem-persistent-critic.md) is complete: its full-stack cost screen fails on all three seeds, and the separate visitation check exposes a large self-play/postflop coverage gap. **Next: test selective additional branching at measured cost and audit coverage across positions** before choosing the next online collector change. All artifacts remain local; public hosting and v0.5 release packaging are still ahead. No production model has been promoted, and the v1.0 professional standard remains unproven.
