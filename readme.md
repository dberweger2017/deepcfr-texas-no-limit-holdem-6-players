...
| Full Hold’em learning | Legal bet candidates, all-role self-play, recovery and substantial training remain ahead. Legacy trainers and play interfaces still exist. |

**Next step:** generate legal bet candidates with a distinct regret/value target for each choice. The [current-hand decision encoder](docs/holdem-encoding.md) is implemented and tested, including variable seats, complete betting history and suit/seat symmetry. Before another learning campaign, the snapshot-average path still needs full training/recovery integration and a fresh readiness protocol. The [previous confirmation](docs/reports/neural-readiness.md) remains failed; substantial training requires both a readiness pass and the corrected no-limit pipeline. No model has been promoted.
