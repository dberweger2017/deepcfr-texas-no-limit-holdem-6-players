...
| Full Hold’em learning | All-role self-play, recovery and substantial training remain ahead. Legacy trainers and play interfaces still exist. |

**Next step:** build the all-role self-play collector around the tested [decision encoder](docs/holdem-encoding.md) and [legal betting interface](docs/holdem-betting.md). Each traverser bet needs its own payoff estimate under a consistent policy profile; learning to prefer good sizes remains ahead. Before another learning campaign, the snapshot-average path still needs full training/recovery integration and a fresh readiness protocol. The [previous confirmation](docs/reports/neural-readiness.md) remains failed; substantial training requires both a readiness pass and the corrected no-limit pipeline. No model has been promoted.
