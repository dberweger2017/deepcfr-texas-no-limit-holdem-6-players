...
**Next step:** compare a bounded-work traversal design, then establish a meaningful multi-seed Hold'em learning baseline before scaling. [Collection profiling](docs/reports/holdem-collection-performance.md) reduced repeated computation but confirmed that the difficult traversal also has excessive branching. Step 4's engineering supports collection → fitting → averaged play → evaluation and exact recovery. Its [first report](docs/reports/holdem-baseline.md) remains a short implementation check: ten jobs completed, one hit its collection limit and one was not attempted. The [snapshot readiness report](docs/reports/snapshot-readiness.md) closes the small-game gate; the [previous neural-average confirmation](docs/reports/neural-readiness.md) remains failed. Hold'em learning and playing strength remain unproven. No model has been promoted.
